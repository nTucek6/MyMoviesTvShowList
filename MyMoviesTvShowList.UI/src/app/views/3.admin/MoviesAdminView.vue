      ...
      <table class="table table-striped">
         ...
         <tbody>
            <tr v-for="m in MovieList">
               <td>{{ m.movieName }}</td>
               <td><span v-for="(g, index) in m.genres">{{ g.label }}<span v-if="index !== m.genres.length - 1">, </span>
                  </span></td>
               <td>{{ m.duration }}</td>
               <td><span v-for="(a, index)  in m.actors">{{ a.firstName }} {{ a.lastName }}<span
                        v-if="index !== m.actors.length - 1">, </span> </span></td>
               <td><span v-for="(d, index)  in m.director">{{ d.firstName }} {{ d.lastName }}<span
